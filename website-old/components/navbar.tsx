...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // If no section is active (e.g., at the very top), set to home
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-2">
              <div className="w-[30px] h-[30px] bg-gray-700 rounded animate-pulse" />
              <div className="w-24 h-6 bg-gray-700 rounded animate-pulse" />
            </div>
            <div className="hidden md:flex space-x-4">
              {navLinks.map((_, i) => (
                <div key={i} className="w-20 h-6 bg-gray-700 rounded animate-pulse" />
              ))}
            </div>
          </div>
        </div>
