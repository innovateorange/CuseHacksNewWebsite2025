...
const getStoredTeamMembers = () => {
  ...
  const stored = localStorage.getItem('cusehacks_team_members')
  if (!stored) {
...
const setStoredTeamMembers = (members: any[]) => {
  if (typeof window !== 'undefined') {
    localStorage.setItem('cusehacks_team_members', JSON.stringify(members))
...
const mockAPIImpl = {
  ...
  login: async (email: string, password: string) => {
    ...
    if (email === 'admin' && password === '0hjzaExb4uU7aJJ2Xi49') {
  ...
  // Registration management (mock implementations)

  updateRegistrationStatus: async (registrationId: string, status: 'pending' | 'confirmed' | 'waitlisted') => {
    await new Promise(resolve => setTimeout(resolve, 300))
    const registration = mockRegistrations.find(r => r._id === registrationId)
    if (registration) {
  ...
  deleteRegistration: async (registrationId: string) => {
    await new Promise(resolve => setTimeout(resolve, 300))
    const index = mockRegistrations.findIndex(r => r._id === registrationId)
    if (index > -1) {
