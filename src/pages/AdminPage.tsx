...
function AdminPage() {
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      const { isCurrentMember, ...memberFields } = newMember
      const memberData = {
        ...memberFields,
        startYear: isCurrentMember ? new Date().getFullYear() : 2023,
        _id: Date.now().toString(),
        isActive: true
      }
      console.log('Sending team member data:', memberData)
  ...
  const handleUpdateMember = async (e: React.FormEvent) => {
    ...
    try {
      const { _id, ...updateData } = editingMember
      const memberUpdate = {
        id: _id,
        ...updateData
      }
      console.log('Updating team member with data:', memberUpdate)
      await mockAPI.updateTeamMember(memberUpdate)
      setTeamMembers(prev =>
        ...
        {showAddForm && (
          ...
          >
            ...
            <form onSubmit={handleAddMember} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-3">Member Status</label>
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      name="memberStatus"
                      checked={newMember.isCurrentMember}
                      onChange={() => setNewMember(prev => ({ ...prev, isCurrentMember: true }))}
                      className="text-primary-500 focus:ring-primary-500"
                    />
                    <span className="text-white">Current Member</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      name="memberStatus"
                      checked={!newMember.isCurrentMember}
                      onChange={() => setNewMember(prev => ({ ...prev, isCurrentMember: false }))}
                      className="text-primary-500 focus:ring-primary-500"
                    />
                    <span className="text-white">Past Member</span>
                  </label>
                </div>
              </div>
                ...
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      checked={editingMember.startYear >= new Date().getFullYear() - 1}
                      onChange={() => setEditingMember(prev => prev ? ({ ...prev, startYear: new Date().getFullYear(), isActive: true }) : null)}
                      className="text-primary-500 focus:ring-primary-500"
                    />
                    <span className="text-white">Current Member</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      name="editMemberStatus"
                      checked={editingMember.startYear < new Date().getFullYear() - 1}
                      onChange={() => setEditingMember(prev => prev ? ({ ...prev, startYear: 2023, isActive: false }) : null)}
                      className="text-primary-500 focus:ring-primary-500"
                    />
                    <span className="text-white">Past Member</span>
                  </label>
